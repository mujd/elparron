...
REST_ROUTER.prototype.handleRoutes = function (router, connection, md5) {
	...
	router.get("/programacionDiaria/imprimir/:fecha/:sucursal_id", function (req, res) {
		connection.query(`
            SELECT  id FROM programacionDiariaCab
            WHERE   fecha = ` + req.params.fecha + ` AND 
                    sucursal_id = ` + req.params.sucursal_id, function (err, rows) {
			if (err) {
				res.json({
					"error": err
				});
