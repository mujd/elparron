...
function programacionDiariaNormalFechaImprimir() {
	var datos = prograDiariaCabSerializar();
	alert(JSON.stringify(datos));
	if (parseInt(datos.programacionDiariaCab_id) > 0) {
		ajaxPut(rutaURL + "/programacionDiaria/normal/imprimir/" + datos.programacionDiariaCab_id, datos);
		ajaxPut(rutaURL + "/programacionDiaria/pedido/imprimir/" + datos.programacionDiariaCab_id, datos);
		ajaxPut(rutaURL + "/programacionDiaria/especial/imprimir/" + datos.programacionDiariaCab_id, datos);
	}
}
...
/* Obtener Programacion Diaria Cab ID */

function obtenerProDiariaCabId() { // Solicita a la API la id de la programacionDiariaCab
	var programacionDiariaCabFecha = $("#dateProgramacionDiaria").val().split("/")[2] + $("#dateProgramacionDiaria").val().split("/")[1] + ($("#dateProgramacionDiaria").val().split("/")[0]);
	var pdCabSucursal = $("#cmbProgramacionDiariaSucursal").val();
	ajaxGet(rutaURL + "/programacionDiaria/imprimir/" + programacionDiariaCabFecha + "/" + pdCabSucursal, desplegarProDiaCabId);
	// obtenerPrecioTorta();
}

function desplegarProDiaCabId(oData) { // Despliega la id de la torta
$("#ulProgramacionDiariaCab li").remove();
	$(oData).each(function() {
			$("#ulProgramacionDiariaCab").append("<li ><input id='txtCabId' type='text' value='" + this.id + "'></li>");
	});
}

function prograDiariaCabSerializar(){
	datos = {
		"programacionDiariaCab_id": $("#txtCabId").val()
	}
	return datos;
}